...
namespace Flush {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			FLUSH_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		FLUSH_CORE_INFO("Loading mesh: {0}", filename.c_str());

		m_Importer = std::make_unique<Assimp::Importer>();

		const aiScene* scene = m_Importer->ReadFile(filename, s_MeshImportFlags);
		if (!scene || !scene->HasMeshes())
			FLUSH_CORE_ERROR("Failed to load mesh file: {0}", filename);
		...
		m_Material.reset(new Flush::Material(m_MeshShader));
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			FLUSH_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			FLUSH_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				FLUSH_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						FLUSH_CORE_TRACE("Found existing bone in map");
	...
	{
		//FLUSH_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		//FLUSH_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		//FLUSH_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		FLUSH_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		//FLUSH_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		//FLUSH_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		FLUSH_CORE_TRACE("------------------------------------------------------");
		FLUSH_CORE_TRACE("Vertex Buffer Dump");
		FLUSH_CORE_TRACE("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				FLUSH_CORE_TRACE("Vertex: {0}", i);
				FLUSH_CORE_TRACE("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				FLUSH_CORE_TRACE("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				FLUSH_CORE_TRACE("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				FLUSH_CORE_TRACE("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				FLUSH_CORE_TRACE("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				FLUSH_CORE_TRACE("--");
			}
		}
		else
