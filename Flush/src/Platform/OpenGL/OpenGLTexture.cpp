...
namespace Flush {
	...
	static GLenum FlushToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
		case Flush::TextureFormat::RGB:     return GL_RGB;
		case Flush::TextureFormat::RGBA:    return GL_RGBA;
		}
		return 0;
	}
		...
		: m_FilePath(path)
	{
		...
		FLUSH_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
		...
		Renderer::Submit([this, srgb]()
		{
			...
			if (srgb)
			{
				...
				FLUSH_CORE_INFO("Creating srgb texture width {0} mips", levels);
	...
	{
		FLUSH_CORE_ASSERT(m_Locked, "Texture must be locked!");
